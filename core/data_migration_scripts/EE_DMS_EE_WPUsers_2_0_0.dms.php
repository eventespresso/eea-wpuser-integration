...
class EE_DMS_EE_WPUsers_2_0_0 extends EE_Data_Migration_Script_Base
{
    ...
    {
        $version_string = $current_database_state_of[ $this->slug() ] ?? false;

        global $wpdb;
        // let's also determine if migrations are needed by the presence of any EE_Attendee_ID
        // relations in the db.  If there are none then no migrations needed! /** @type WPDB */ global $wpdb;
        $has_records = $wpdb->get_var(
            "SELECT COUNT('user_id') FROM $wpdb->usermeta WHERE meta_key LIKE '%EE_Attendee_ID%'"
        );
