...
<tr>
    ...
    <td>
        ...
        $actions['edit_registration'] = $registration->count_question_groups()
            ? '<a aria-label="' . __('Link to edit registration',
                'event_espresso') . '" href="' . $registration->edit_attendee_information_url() . '">'
        ...
        if ($registration->is_primary_registrant() ||
            ...
            $actions['resend_registration'] = '<a aria-label="' . __('Link to resend registration message',
                    'event_espresso') . '" href="' . $resend_registration_link . '">'
        ...
        if ($registration->is_primary_registrant()
            ...
            $actions['make_payment'] = '<a aria-label="' . __('Link to make payment',
                    'event_espresso') . '" href="' . $registration->payment_overview_url() . '">'
                                       . '<span class="dashicons dashicons-cart"></span></a>';
        }

        //receipt link?
        if ($registration->is_primary_registrant() && $registration->receipt_url()) {
            $actions['receipt'] = '<a aria-label="' . __('Link to view receipt',
                    'event_espresso') . '" href="' . $registration->receipt_url() . '">'
        ...
        if ($registration->is_primary_registrant() && $registration->invoice_url()) {
            $actions['invoice'] = '<a aria-label="' . __('Link to view invoice',
                    'event_espresso') . '" href="' . $registration->invoice_url() . '">'
